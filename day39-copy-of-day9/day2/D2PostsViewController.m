...
-(IBAction) pressedShareButton
{
    [Flurry logEvent:@"userPressedShareButton"];
    ...
    if( selectedRow ) {
        ...
        [request startWithCompletionHandler:^(FBRequestConnection *connection, id result, NSError *error) {
            NSLog( @"%@ %@", result, error );
            if( !error ) {
                [Flurry logEvent:@"userShareSuccess"];
            }
        }];
...
-(IBAction) pressedFBButton
{
    ...
    [session openWithBehavior:FBSessionLoginBehaviorUseSystemAccountIfPresent
            completionHandler:^(FBSession *session,
                ...
                [Flurry logEvent:@"userPressedShareButton" withParameters:@{@"facebookId": [FBSession activeSession].appID}];
    }];
}
